...
for epoch in range(args.epochs):
    ...
    if loss_val <= best_val_loss or acc_val >= best_val_acc:
        if loss_val <= best_val_loss and acc_val >= best_val_acc:
            ...
            print('save weights')
            epoch_save = epoch
        # print(acc_val.__class__)
        best_val_acc = max((best_val_acc, acc_val))
        best_val_loss = min((best_val_loss, loss_val))
        # best_val_acc = np.max((best_val_acc.cpu().detach().numpy(), acc_val.cpu().detach().numpy()))
        # best_val_loss = np.min((best_val_loss.cpu().detach().numpy(), loss_val.cpu().detach().numpy()))
        cnt = 0
    else:
        cnt += 1
